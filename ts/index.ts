...
export class Batcher<I, O> {
  ...
  constructor(options: BatcherOptions<I, O>) {
    ...
    if (Array.isArray(options.queuingThresholds)) {
      if (!options.queuingThresholds.length) {
        throw new Error(
          "options.batchThresholds must contain at least one number",
        );
      }
      options.queuingThresholds.forEach((n) => {
        if (n < 1) {
          throw new Error(
            "options.batchThresholds must only contain numbers greater than 0",
          );
        }
      });
    ...
    if (!isNull(options.queuingDelay)) {
      if (options.queuingDelay < 0) {
        throw new Error(
          "options.queuingDelay must be greater than or equal to 0",
        );
      }
  ...
  public getResult(input: I): Promise<O> {
    ...
    return new Promise<O>(
      function(resolve, reject) {
        this._outputQueue[index] = { resolve, reject };
        this._trigger();
      }.bind(this),
    );
  }
  ...
  private _trigger(): void {
    ...
    const thresholdIndex: number = Math.min(
      ...
      this._queuingThresholds.length - 1,
    );
      ...
      if (!isNull(result)) {
        ...
        resultPromise
          ...
          .catch((err) => {
            ...
            const promises = this._outputQueue.splice(
              0,
